...
(function (factory) {
  ...
  var EnjoyHint = function(configs) {
    ...
    var stepAction = function(isPrev) {
      ...
      if (
        ...
        typeof step_data.onBeforeStart === "function"
          ...
          step_data.onBeforeStart(isPrev);//is stepAction raised due to Previous button click
      }
      ...
      setTimeout(function() {
        if (!step_data.selector) {
          ...
          //if current selector is hidden then move to next/prev control
          if (!$(step_data.selector).is(":visible")) {
            setTimeout(function () {
                if (isPrev) {
                    prevStep();
                }
        ...
        if(isHintInViewport.top < 0 || isHintInViewport.bottom > (window.innerHeight || document.documentElement.clientHeight)){
            hideCurrentHint();
            if ($(".scrollable-div").length > 0 && $(step_data.selector, $(".scrollable-div")).length > 0 ) {
                OverlayScrollbars($(".scrollable-div")).scroll({ el: $(step_data.selector) }); //https://github.com/KingSora/OverlayScrollbars
            }
            else {
        ...
        $(window).on("resize", function() {
          ...
          doit = setTimeout(function() {
              if (boundingClientRect.top < 0 || boundingClientRect.bottom > (window.innerHeight || document.documentElement.clientHeight)) {
                  if ($(".scrollable-div").length > 0 && $(step_data.selector, $(".scrollable-div")).length > 0) {
                      OverlayScrollbars($(".scrollable-div")).scroll({ el: $(that.stepData.enjoyHintElementSelector) }); //https://github.com/KingSora/OverlayScrollbars
                  }
                  else {
